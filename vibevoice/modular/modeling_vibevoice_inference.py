...
class VibeVoiceForConditionalGenerationInference(VibeVoicePreTrainedModel, GenerationMixin):
    ...
    def tie_weights(self, *args, **kwargs):
        ...
        should_tie = (
            ...
            or getattr(getattr(self.config, "decoder_config", None), "tie_word_embeddings", False)
        )
        if not should_tie:
            return
        ...
        **kwargs,
        ...
        if speech_tensors is not None and speech_masks is not None:
            ...
            if speech_input_mask is not None:
                # Robustly align mask/embedding lengths across tokenizer versions.
                mask = speech_input_mask
                seq_len = inputs_embeds.shape[1]
                if mask.shape[1] > seq_len:
                    mask = mask[:, :seq_len]
                elif mask.shape[1] < seq_len:
                    pad = torch.zeros(
                        (mask.shape[0], seq_len - mask.shape[1]),
                        dtype=mask.dtype,
                        device=mask.device,
                    )
                    mask = torch.cat([mask, pad], dim=1)

                target_positions = mask.nonzero(as_tuple=False)
                if target_positions.numel() > 0:
                    n = min(target_positions.shape[0], speech_embeds.shape[0])
                    target_positions = target_positions[:n]
                    inputs_embeds[target_positions[:, 0], target_positions[:, 1]] = speech_embeds[:n]
    ...
    def _build_generate_config_model_kwargs(
        ...
        **kwargs,
        ...
        prepare_cache_sig = inspect.signature(self._prepare_cache_for_generation)
        prepare_cache_param_names = prepare_cache_sig.parameters.keys()

        cache_args = [generation_config, model_kwargs]
        if "assistant_model" in prepare_cache_param_names:
            cache_args.append(assistant_model)
        elif "generation_mode" in prepare_cache_param_names:
            try:
                generation_mode = generation_config.get_generation_mode(assistant_model=assistant_model)
            except TypeError:
                generation_mode = generation_config.get_generation_mode()
            cache_args.append(generation_mode)
        cache_args.extend([batch_size, max_cache_length])
        if "device" in prepare_cache_param_names:
            cache_args.append(device)

        self._prepare_cache_for_generation(*cache_args)
    ...
    def generate(
        ...
        **kwargs,
        ...
        def iter_kv_cache(cache_obj):
            # transformers <=4.x style
            if hasattr(cache_obj, "key_cache") and hasattr(cache_obj, "value_cache"):
                return zip(cache_obj.key_cache, cache_obj.value_cache)
            # transformers 5.x DynamicCache style
       ...
       # Define all valid tokens that can be generated
        ...
        for step in progress_bar:
            ...
            if diffusion_start_indices.numel() > 0 and kwargs.get('refresh_negative', True):
                ...
                for layer_idx, (k_cache, v_cache) in enumerate(iter_kv_cache(negative_model_kwargs['past_key_values'])):
