...
# Ensure local package import works when executed as `python demo/inference_from_file.py`.
...
_NUMBA_CACHE_DIR = os.path.join(_REPO_ROOT, "runtime", "numba_cache")
...
_RUNTIME_LIBS = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "runtime", "pylibs")
)
if os.path.isdir(_RUNTIME_LIBS) and _RUNTIME_LIBS not in os.sys.path:
    os.sys.path.insert(0, _RUNTIME_LIBS)
...
def parse_args():
    default_model_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "checkpoints", "VibeVoice-1.5B")
    )
    default_tokenizer_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "checkpoints", "Qwen2.5-1.5B-tokenizer")
    )
    ...
    parser.add_argument(
        ...
        help="Local tokenizer path (recommended for offline runs). Falls back to model config if missing.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of DDPM inference steps (default: 20)",
    )
    parser.add_argument(
...
def main():
    ...
    processor_kwargs = {}
    if args.tokenizer_path and os.path.isdir(args.tokenizer_path):
        processor_kwargs["language_model_pretrained_name"] = args.tokenizer_path
        processor_kwargs["local_files_only"] = True
        print(f"Using local tokenizer: {args.tokenizer_path}")
    else:
        print("Local tokenizer path not found; processor may try to access Hugging Face Hub.")
    processor = VibeVoiceProcessor.from_pretrained(args.model_path, **processor_kwargs)
    ...
    try:
        ...
        if has_accelerate and args.device in ("cuda", "cpu"):
        ...
        if not has_accelerate and args.device in ("cuda", "mps"):
    ...
    print(f"Using inference_steps={args.inference_steps}, cfg_scale={args.cfg_scale}, refresh_negative={refresh_negative}")
    ...
    if outputs.speech_outputs and outputs.speech_outputs[0] is not None:
        ...
        print("Skipped saving audio because generation did not return waveform data.")
    ...
    print(f"Total tokens: {output_tokens}")
    print(f"Generation time: {generation_time:.2f} seconds")
    if audio_duration is not None and rtf is not None:
