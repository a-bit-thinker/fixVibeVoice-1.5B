...
# Ensure local package import works when executed as `python demo/gradio_demo.py`.
_REPO_ROOT = Path(__file__).resolve().parents[1]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

# Ensure numba/librosa cache is writable in locked-down environments.
_NUMBA_CACHE_DIR = _REPO_ROOT / "runtime" / "numba_cache"
_NUMBA_CACHE_DIR.mkdir(parents=True, exist_ok=True)
os.environ.setdefault("NUMBA_CACHE_DIR", str(_NUMBA_CACHE_DIR))

# Allow local dependency overlays (e.g. diffusers/ml_collections) without touching the base venv.
_RUNTIME_LIBS = _REPO_ROOT / "runtime" / "pylibs"
if _RUNTIME_LIBS.is_dir() and str(_RUNTIME_LIBS) not in sys.path:
    sys.path.insert(0, str(_RUNTIME_LIBS))
...
from gradio.http_server import Server as GradioServer
...
class VibeVoiceDemo:
    def __init__(
        ...
        exclude_bgm_voices: bool = True,
    ):
        """Initialize the VibeVoice demo with model loading."""
        self.model_path = model_path
        self.device = device
        self.inference_steps = inference_steps
        self.refresh_negative = refresh_negative
    ...
    def load_model(self):
        ...
        model_path_obj = Path(self.model_path).expanduser()
        if not model_path_obj.is_absolute():
            repo_relative_model = (_REPO_ROOT / model_path_obj).resolve()
            if repo_relative_model.exists():
                model_path_obj = repo_relative_model
            else:
        ...
        if version.parse(transformers.__version__) >= version.parse("5.0.0"):
            print(
                f"WARNING: transformers {transformers.__version__} is unsupported for VibeVoice-TTS. "
                "Recommended: transformers>=4.51.3,<5.0.0. Unsupported versions can produce gibberish or random syllables."
            )
        ...
        if self.tokenizer_path:
            tokenizer_path_obj = Path(self.tokenizer_path).expanduser()
            tokenizer_candidates = [tokenizer_path_obj]
            if not tokenizer_path_obj.is_absolute():
                tokenizer_candidates.append(_REPO_ROOT / tokenizer_path_obj)
            for candidate in tokenizer_candidates:
                if candidate.is_dir():
                    tokenizer_path_resolved = str(candidate.resolve())
                    break

        if tokenizer_path_resolved:
            self.tokenizer_path = tokenizer_path_resolved
            processor_kwargs["language_model_pretrained_name"] = self.tokenizer_path
            processor_kwargs["local_files_only"] = True
            print(f"Using local tokenizer: {self.tokenizer_path}")
        else:
            print("Local tokenizer path not found; processor may try to access Hugging Face Hub.")
        self.processor = VibeVoiceProcessor.from_pretrained(self.model_path, **processor_kwargs)
        ...
        try:
            ...
            print("accelerate is not installed; loading model without `device_map`.")
        ...
        if self.device == "mps":
            ...
            if importlib.util.find_spec("flash_attn") is None:
                print("flash_attn is not installed; using SDPA instead of flash_attention_2.")
                attn_impl_primary = "sdpa"
        else:
            load_dtype = torch.float32
            attn_impl_primary = "sdpa"
        ...
        try:
            ...
            if has_accelerate and self.device in ("cuda", "cpu"):
            ...
            if not has_accelerate and self.device in ("cuda", "mps"):
            ...
            if attn_impl_primary == 'flash_attention_2':
                ...
                if has_accelerate and self.device in ("cuda", "cpu"):
                    fallback_kwargs["device_map"] = self.device
                elif self.device == "mps":
    ...
    def setup_voice_presets(self):
        ...
        # Exclude presets known to contain background music by default.
        if self.exclude_bgm_voices:
            before_filter = len(self.voice_presets)
            self.voice_presets = {
                name: path for name, path in self.voice_presets.items()
                if "_bgm" not in name.lower()
            }
            filtered_count = before_filter - len(self.voice_presets)
            if filtered_count > 0:
                print(f"Filtered out {filtered_count} *_bgm voice preset(s) to avoid intro music.")
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith('Speaker ') and ':' in line:
                    ...
                    # Unlabeled lines default to speaker 0 for stable single-speaker behavior.
                    speaker_id = 0
    ...
    with gr.Blocks() as interface:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_classes="settings-card"):
                ...
                default_speakers = ['en-Carter_man', 'en-Frank_man', 'en-Maya_woman', 'en-Alice_woman']
...
Or paste text directly and it will auto-assign speakers.""",
        ...
        # Gradio Examples internally uses multiprocessing locks in this environment.
        # We keep the Random Example button and skip gr.Examples to avoid startup failure.
        gr.Markdown("### 📚 **Example Scripts**\nUse **🎲 Random Example** to load a prepared script.")
...
def parse_args():
    default_model_path = str((_REPO_ROOT / "checkpoints" / "VibeVoice-1.5B").resolve())
    default_tokenizer_path = str((_REPO_ROOT / "checkpoints" / "Qwen2.5-1.5B-tokenizer").resolve())
    ...
    parser.add_argument(
        ...
        help="Local tokenizer path (recommended for offline runs).",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--disable_refresh_negative",
        action="store_true",
        default=False,
        help="Disable negative-branch cache refreshing (usually keep enabled).",
    )
    parser.add_argument(
        "--share",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0.0.0",
        help="Host to bind the demo server to (use 0.0.0.0 for LAN access).",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Include voice presets with '_bgm' in filename. Disabled by default to reduce intro music.",
    )
...
def main():
    ...
    demo_instance = VibeVoiceDemo(
        ...
        refresh_negative=not args.disable_refresh_negative,
        tokenizer_path=args.tokenizer_path,
        adapter_path=args.checkpoint_path,
        exclude_bgm_voices=not args.allow_bgm_voices,
    )
    
    # Create interface
    interface = create_demo_interface(demo_instance)
    
    print(f"🚀 Launching demo on {args.host}:{args.port}")
    ...
    try:
        ...
        try:
            ...
            # Some constrained environments fail inside Gradio's internal port probe.
            if "Cannot find empty port in range" not in str(exc):
                raise
            print("Gradio default launcher failed; falling back to direct uvicorn server startup.")
            print("This fallback still serves the same app and supports LAN access.")
            app = GradioApp.create_app(queued_interface, mcp_server=False)
            config = uvicorn.Config(app=app, host=args.host, port=args.port, log_level="warning")
            server = GradioServer(config=config)
            server.run()
